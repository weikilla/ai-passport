...
void demo_wooden_fish_enter(void) {
    ...
    // Create screen (uses 240x320 coordinate system)
    s_screen = ui_pixel_screen_create("FISH");
    
    // Main panel in center
    lv_obj_t *panel = ui_pixel_panel_create(s_screen, 18, 50, 204, 200, UI_PAPER);
    
    // Fish body - oval wooden block
    s_fish_body = lv_obj_create(panel);
    lv_obj_set_size(s_fish_body, 100, 80);
    lv_obj_set_style_bg_color(s_fish_body, lv_color_hex(0x8B4513), 0);
    lv_obj_set_style_radius(s_fish_body, 35, 0);
    lv_obj_set_style_border_width(s_fish_body, 3, 0);
    lv_obj_set_style_border_color(s_fish_body, lv_color_hex(0xD2691E), 0);
    lv_obj_align(s_fish_body, LV_ALIGN_TOP_MID, 0, 20);
    
    // Fish center circle
    lv_obj_t *center = lv_obj_create(s_fish_body);
    lv_obj_set_size(center, 30, 30);
    lv_obj_set_style_bg_color(center, lv_color_hex(0xD2691E), 0);
    lv_obj_set_style_radius(center, 15, 0);
    ...
    lv_obj_align(s_label_merit, LV_ALIGN_BOTTOM_MID, 0, -50);
    ...
    lv_obj_align(s_label_combo, LV_ALIGN_BOTTOM_MID, 60, -20);
